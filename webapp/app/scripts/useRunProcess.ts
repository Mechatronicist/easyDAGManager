...
export type ProcessStatusKey = keyof typeof ProcessStatus
export type ProcessStatusValue = (typeof ProcessStatus)[ProcessStatusKey]
...
export function useRunProcess({
  ...
  const graph = computed<Graph>(() => toValue(dagreGraph))
...
async function until(condition: () => boolean): Promise<true> {
  return new Promise((resolve) => {
